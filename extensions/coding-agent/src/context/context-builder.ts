...
import { ContextExpansionEngine, ContextNode, ExpansionResult } from './context-expansion';
...
export class ContextBuilder {
  ...
  async build(userRequest: string, currentFile?: string, gitContext?: string, verificationResults?: string): Promise<ContextPackage> {
    ...
    if (expansionResult) {
      // New flow: build from expansion results
    ...
    // Defensive filter: remove deleted files (index may not be synced timely)
  ...
  formatForPrompt(pkg: ContextPackage): string {
    ...
    if (pkg.primaryFiles.length > 0) {
      const lines = pkg.primaryFiles.map(f => `  [FILE] ${this.shortenPath(f)}`);
      budgetMgr.addChunk('file', 'Primary Files', `### Primary Files\n${lines.join('\n')}`, { priority: 65 });
    }

    // Related Files
    if (pkg.relatedFiles.length > 0) {
      const lines = pkg.relatedFiles.map(f => `  [FILE] ${this.shortenPath(f)}`);
      budgetMgr.addChunk('file', 'Related Files', `### Related Files (dependents)\n${lines.join('\n')}`, { priority: 60 });
    }

    // Dependency Files
    if (pkg.dependencyFiles.length > 0) {
      const lines = pkg.dependencyFiles.map(f => `  [FILE] ${this.shortenPath(f)}`);
  ...
  async buildIncremental(
    ...
    // Symbol Retrieval + Context Expansion (also supports incremental mode)
    let expansionResult: ExpansionResult | undefined;
    let primarySymbols: SymbolEntry[] = [];
  ...
  buildProjectContext(request: string): ProjectContextPackage {
    ...
    // Use primary workspace files to avoid context pollution in multi-workspace scenarios
    ...
    // Project introduction questions prioritized with source code evidence; README as supplement
  ...
  formatProjectContextForPrompt(pkg: ProjectContextPackage): string {
    ...
    if (pkg.architectureFiles.length > 0) {
      ...
      for (const f of pkg.architectureFiles) {
        parts.push(`  [ARCH] ${this.shortenPath(f)}`);
      }
      parts.push('');
    }
    ...
    if (pkg.buildFiles.length > 0) {
      ...
      for (const f of pkg.buildFiles) {
        parts.push(`  [BUILD] ${this.shortenPath(f)}`);
      }
      parts.push('');
    }
    ...
    if (pkg.serverModules.length > 0) {
      ...
      for (const f of pkg.serverModules) {
        parts.push(`  [SRV] ${this.shortenPath(f)}`);
      }
      parts.push('');
    }
    ...
    if (pkg.coreModules.length > 0) {
      ...
      for (const f of pkg.coreModules) {
        parts.push(`  [CORE] ${this.shortenPath(f)}`);
      }
      parts.push('');
    }
    ...
    if (pkg.entryPoints.length > 0) {
      ...
      for (const f of pkg.entryPoints) {
        parts.push(`  [ENTRY] ${this.shortenPath(f)}`);
      }
      parts.push('');
    }
  ...
  private classifyProjectIntent(request: string): string {
    ...
    if (/microservice|service|deploy|cluster|container|docker|k8s|kubernetes/i.test(lower)) {
      return 'deployment';
    }
    if (/architecture|design|overview|structure/i.test(lower)) {
      return 'architecture';
    }
    if (/config|configuration|setup|install/i.test(lower)) {
      return 'configuration';
    }
    if (/test|ci|cd|pipeline|integration/i.test(lower)) {
      return 'ci_cd';
    }
  ...
  classifyIntent(request: string): string {
    ...
    if (/about this project/i.test(lower)) {
      return 'project_understanding';
    }
    if (/introduce|explain|what.*project|project.*about|project.*function|project.*purpose/i.test(lower)) {
      return 'project_understanding';
    }
    if (/view.*project|current.*project|project.*overview|project.*summary|project.*status/i.test(lower)) {
      return 'project_understanding';
    }
    if (/project.*structure|directory.*structure|project.*architecture|analyze.*project|analyze.*arch|project.*organization/i.test(lower)) {
      return 'project_understanding';
    }
    if (/project.*feature|project.*module|project.*function|project.*description|project.*usage/i.test(lower)) {
      return 'project_understanding';
    }
    if (/淇|fix|bug|error|issue|crash|fail|not work|wrong|broken/i.test(lower)) {
      return 'bug_fix';
    }
    if (/娣诲姞|澧炲姞|鏂板|new|add|create|implement|feature/i.test(lower)) {
      return 'feature_add';
    }
    if (/閲嶆瀯|refactor|clean|improve|optimize|restruct/i.test(lower)) {
      return 'refactor';
    }
    if (/remove|delete|drop/i.test(lower)) {
      return 'removal';
    }
    if (/娴嬭瘯|test|spec|unit|integration/i.test(lower)) {
      return 'testing';
    }
    if (/鏂囨。|doc|readme|comment|documentation|explain/i.test(lower)) {
      return 'documentation';
    }
    if (/search|web search|look up|google|bing|stackoverflow|github\.com|npmjs|pypi|docs\./i.test(lower)) {
      return 'other';
    }
