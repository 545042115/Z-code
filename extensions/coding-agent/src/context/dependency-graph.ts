...
export class DependencyGraph {
  ...
  /**
   * 杩斿洖鏂扮殑姝ｅ垯瀹炰緥锛岄伩鍏嶅甫 g 鏍囧織鐨勬鍒欏湪骞跺彂璋冪敤涓叡浜?lastIndex 瀵艰嚧绔炴€佹潯浠躲€?   */
  ...
  formatForPrompt(filePath?: string, maxDepth: number = 2): string {
    ...
    if (filePath) {
      ...
      for (const dep of node.dependencies.slice(0, 10)) {
        ...
        lines.push(`    鈫?${this.shortenPath(dep)} (used by ${depCount} files)`);
      }
    ...
    for (const c of critical) {
      lines.push(`  ${this.shortenPath(c.filePath)} 鈥?${c.dependentCount} dependents`);
    }
