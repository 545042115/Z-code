...
export class RepoGraph {
  ...
  getDependencyOverview(): string {
    ...
    parts.push('  Request 鈫?Entry Point 鈫?Server Layer 鈫?Core Layer 鈫?Response');
    parts.push('  UI Layer 鈫?Server Layer 鈫?Core Layer 鈫?Data');
    parts.push('  Config/Build 鈫?All Layers (cross-cutting)\n');

    if (this.dataFlows.length > 0) {
      parts.push('### Key Data Flows\n');
      for (const flow of this.dataFlows.slice(0, 5)) {
        const pathStr = flow.through.map(t => this.shortenName(t)).join(' 鈫?');
        parts.push(`  ${this.shortenName(flow.from)} 鈫?${pathStr} 鈫?${this.shortenName(flow.to)}`);
  ...
  formatForPrompt(): string {
    ...
    for (const edge of crossEdges) {
      ...
      const key = `${sourceNode.moduleTag}鈫?{targetNode.moduleTag}`;
      ...
      if (entry.files.length < 3) {
        entry.files.push(`${sourceNode.name} 鈫?${targetNode.name}`);
      }
    }
      ...
      for (const server of serverNodes) {
        if (this.hasPath(entry.id, server.id)) {
          ...
          this.dataFlows.push({
            ...
            to: server.id,
            through,
            description: `Request entry 鈫?server handler`,
          });
        }
      }
    }
      ...
      for (const core of coreNodes) {
        if (this.hasPath(server.id, core.id)) {
          ...
          this.dataFlows.push({
            ...
            to: core.id,
            through,
            description: `Server 鈫?core logic`,
          });
        }
      }
    }
  ...
  private renderHierarchy(
    ...
    if (depth > 0) {
      ...
      const icon = node.type === 'root' ? '' : node.type === 'submodule' ? '馃搧' : '馃搫';
