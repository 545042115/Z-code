// End-to-end integration test: verify that wrapped LLM + Tool produce
// queryable Spans in the Store, matching what the Dashboard will read.
//
// This is the Phase 1 acceptance test: if this passes, the trace layer
// is ready to be wired into the real AgentCore / AgentLoop.
...
import { TraceManager, TraceInstrumentation } from '../index';
import { createFileStore, type Store } from '../../infra/storage';
import type { ToolRegistry } from '../../tools/tool-registry';

// ── Mocks ─────────────────────────────────────────────────────────────
//
// We use duck-typed mocks with locally-defined types to avoid pulling
// the `vscode` module (transitively imported by llm-provider.ts) into
// the test runtime. The instrumentation layer only requires the shape
...
class MockLLM {
  async generate(_req: MockGenerateRequest): Promise<string> {
    return 'This is a mock LLM response.';
  }
  async *generateStream(_req: MockGenerateRequest): AsyncIterable<string> {
    yield 'This ';
    yield 'is ';
    yield 'a ';
    yield 'mock ';
...
test('e2e: wrapped LLM.generate produces an llm Span queryable from Store', async () => {
  await withManager(async (m, store) => {
    ...
    const inst = new TraceInstrumentation({ tracker: t });
    const llm = inst.wrapLLM(new MockLLM() as any, model);
    ...
    // Verify the Span is in the store
    ...
    // Verify the Run has aggregated usage
...
test('e2e: wrapped Tool.execute produces a tool Span', async () => {
  await withManager(async (m, store) => {
    ...
    const inst = new TraceInstrumentation({ tracker: t });
    const tools = inst.wrapToolRegistry(new MockToolRegistry() as unknown as ToolRegistry);
