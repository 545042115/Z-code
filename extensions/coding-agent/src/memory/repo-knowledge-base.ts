...
import { RepoGraph, GraphNode } from '../context/repo-graph';
import { RepoMap } from '../context/repo-map';
...
export class RepoKnowledgeBase {
  ...
  private async applyIncrementalUpdate(): Promise<void> {
    ...
    // Trigger full rebuild if changes are large or config files are involved
    ...
    const modules = Array.from(dirMap.entries())
      .map(([dir, data]) => {
        ...
        return {
          ...
          topExports: exports,
          description: `${data.files.length} files, ${data.symbols.length} symbols`,
        };
      })
      .sort((a, b) => b.fileCount - a.fileCount)
      .slice(0, 20);
  ...
  private buildCodingPatterns(sourceFiles: WorkspaceFile[]): RepoKnowledgeCodingPatterns {
    ...
    if (namingChecks.pascalCase > namingChecks.camelCase) {
      conventions.push('class/interface use PascalCase');
    }
    if (namingChecks.camelCase > 0) {
      conventions.push('function/variable use camelCase');
    }
    ...
    return {
      ...
      fileOrganization: 'Organized by directory/function layers',
      commonImports,
      designPatterns: [],
    };
  }
  ...
  private buildCriticalFiles(): RepoKnowledgeCriticalFile[] {
    ...
    return critical.map((c) => ({
      ...
      reason: `depended by ${c.dependentCount} files`,
    }));
  }
  ...
  private inferLayerDescription(tag: string): string {
    const map: Record<string, string> = {
      server: 'Server/API route and controller layer',
      core: 'Core business logic and utility layer',
      ui: 'UI component and page layer',
      config: 'Configuration and environment variable layer',
      build: 'Build scripts and toolchain layer',
      other: 'Other supporting files',
    };
    return map[tag] || `${tag} module`;
  }

  private inferArchitectureSummary(layers: RepoKnowledgeLayer[]): string {
    const parts: string[] = [];
    const totalFiles = layers.reduce((sum, l) => sum + l.fileCount, 0);
    parts.push(`${totalFiles} source files`);
