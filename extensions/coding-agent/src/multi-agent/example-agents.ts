...
// Real agents wrap an LLMProvider; see `TraceInstrumentation.wrapLLM`.
...
export const CoderAgent: IAgent = {
  ...
  async execute(ctx: TaskContext): Promise<AgentResult> {
    ...
    if (!findings) {
      return fail('3005', "dependency 'researcher' did not produce findings");
    }
...
export const ReviewerAgent: IAgent = {
  ...
  async execute(ctx: TaskContext): Promise<AgentResult> {
    ...
    if (!patch) {
      return fail('3005', "dependency 'coder' did not produce patch");
    }
    const review = {
...
export function registerExampleAgents(reg: import('./agent-registry').AgentRegistry): void {
