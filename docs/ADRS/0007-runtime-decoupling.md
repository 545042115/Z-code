# 0007. Phase 6 — Runtime 解耦：建立 Z Assistant Monorepo
...
| **C. 建立独立 monorepo `z-assistant`**，VSCode Extension 退化为 Connector app | 顶层 monorepo，下含 `packages/*` 与 `apps/*` | 完全满足 ROADMAP；可独立发布；未来可加 desktop/cli/web | 需要跨仓迁移 / git history 保留 |
...
```
z-assistant/                                       # 新顶层仓库（或在 monorepo 根并列）
│
...
│   │   ├── package.json       @z-assistant/contracts
│   │   ├── tsconfig.json
│   │   └── src/
│   │       ├── index.ts
...
│   │   ├── package.json       @z-assistant/trace
│   │   ├── src/
│   │   │   ├── span.ts
│   │   │   ├── run-tracker.ts
│   │   │   ├── instrumentation.ts
...
│   │   ├── package.json       @z-assistant/evaluation
│   │   ├── src/
│   │   │   ├── harness/
│   │   │   │   ├── sandbox.ts
│   │   │   │   ├── rubric.ts
...
│   │
│   ├── evolution/                                 # Phase 5
│   │   ├── package.json       @z-assistant/evolution
│   │   ├── src/
│   │   │   ├── engine.ts
│   │   │   ├── fingerprint.ts
│   │   │   ├── suggestions.ts
...
│   │   ├── package.json       @z-assistant/workflow
│   │   ├── src/
...
│   │   ├── package.json       @z-assistant/memory
│   │   ├── src/
...
│   │   ├── package.json       @z-assistant/tools
│   │   ├── src/
│   │   │   ├── registry.ts
...
│   │   ├── package.json       @z-assistant/connectors
│   │   ├── src/
...
│   │   ├── package.json       @z-assistant/agents
│   │   ├── src/
...
│   │       ├── package.json   @z-assistant/agent-coding
│   │       ├── src/
...
│   │
│   └── runtime/                                   # Phase 6 核心
│       ├── package.json       @z-assistant/runtime
│       ├── src/
...
│   │   ├── package.json   @z-assistant/app-desktop
│   │   ├── src/                        (Electron / Tauri)
│   │   └── webview/                    (复用 packages/trace 渲染逻辑)
│   │
│   ├── vscode-connector/                          # 当前 extension 改造
│   │   ├── package.json   @z-assistant/app-vscode
│   │   ├── src/
...
import { AssistantRuntime } from '@z-assistant/runtime';
...
| **M1. 抽 contracts** | 0.5d | 复制 `src/contracts/*` → `packages/contracts/src/`；在原路径留 `re-export shim`：`src/contracts/index.ts` = `export * from '@z-assistant/contracts'` | 所有 `from '../contracts'` 仍工作 |
| **M2. 抽 infra** | 1d | 同上：`infra/errors`、`infra/cost`、`infra/storage`、`infra/permission`、`infra/config` 逐包搬 | 写 import 路径修正 |
| **M3. 抽 trace** | 1d | `src/trace/*` → `packages/trace/src/`；`src/trace-ui/query-service.ts` 拆为 `packages/trace/src/projections.ts` + `apps/vscode-connector/src/webviews/trace/` | webview 的 `import { QueryService }` 改为 `@z-assistant/trace` |
...
{
  "dependencies": {
    "@z-assistant/contracts": "workspace:*",
    "@z-assistant/trace": "workspace:*",
    // ...
  }
}
```
...
- `pnpm --filter @z-assistant/app-vscode package` —— 打 VSIX
- 手动启动调试扩展，验证：
  1. TracePanel 能打开
...
| 用户既有 V1 配置文件（`.z/agent.json`）路径不兼容 | 中 | 用户体验降级 | M8 时增加 `legacy-config-migrator`；读取老路径自动写到新 `~/.z-assistant/` |
| 数据迁移（旧的 `runs.jsonl` 路径） | 中 | 数据丢失 | `FileStore` 路径改为可配置；M0 引入 `storeDir` 参数；旧数据自动 copy |
| 迁移期间两条 import 路径并存，开发认知负担 | 中 | 误改老路径 | M3 起在 CI 加 lint：`extensions/coding-agent/src/**` 禁止 `import * from '../trace'`（必须 `@z-assistant/trace`） |
...
- [ ] `pnpm --filter @z-assistant/app-vscode package` 成功生成 VSIX
...
2. **包管理器与发布**：`@z-assistant/*` 是否对外发布到 npm？目前**内部 workspaces only** 即可
...
5. **V1 `MemoryManager`（vscode.Memento）** 怎么办？vscode.Memento 只能在 extension host 用 → **应被替换**为 `@z-assistant/memory` 的 `FileMemoryStore`，M6 时同步做
6. **Phase 6 是否同步做 Phase 7 Memory 统一**？建议 **不同步**，分两步走，避免单次 PR 过大
7. **VSIX 包名是否变化**？从 `Z Code` → `Z Assistant`？建议**保留旧名**，避免用户侧更新断裂
