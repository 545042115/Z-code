// WebMcpClient — MCP (Model Context Protocol) Client for Web/Mobile Runtime
//
...
//
// NOTE: `Tool`/`ToolContext`/`ToolResult` below are inlined from the
// mobile runtime's `tools.ts` (which is not migrated because it depends
// on native capabilities). They are structurally identical so the
// returned `Tool[]` instances are compatible with the mobile
// `ToolRegistry.register()` via TypeScript structural typing.
...
/** Tool execution context. Mirrors the mobile runtime's `ToolContext`. */
...
/** Tool execution result. Mirrors the mobile runtime's `ToolResult`. */
...
/** Executable tool. Mirrors the mobile runtime's `Tool`. */
interface Tool {
