// Web/Mobile LLM Provider
//
// OpenAI-compatible chat completions provider for the web/mobile runtime.
// Supports streaming, function/tool calling, and multiple API configs.
//
// NOTE: `ChatMessage` here is the LLM-layer message format (includes
// `tool_calls`), distinct from the UI-layer `ChatMessage` in
// @ziner/contracts. Kept under its original name for compatibility.
...
export class LLMProvider {
  ...
  async chat(
    ...
    options: { tools?: ToolDefinition[]; temperature?: number; maxTokens?: number; signal?: AbortSignal } = {},
