// Unit tests for Evolution engine (V2).
//
// Phase 6A: migrated from V1 `extensions/coding-agent/src/evolution/__tests__/evolution.test.ts`.
// Pure Node, no vscode.
...
test('normalizePattern: replaces numbers, hex, quotes, punctuation', () => {
  // NOTE: <n> and <hex> are checked separately because once the quoted
  // span is replaced by <q>, any digits *inside* that span disappear.
  const pNum = normalizePattern("ENOENT: no such file 1234 at 0xdeadbeef");
  assert.ok(pNum.includes('<n>'), 'numbers replaced');
  assert.ok(pNum.includes('<hex>'), 'hex replaced');
  const pQ = normalizePattern("ENOENT: no such file '/tmp/abc' at 0xdeadbeef");
  assert.ok(pQ.includes('<q>'), 'quotes replaced');
  assert.ok(!pQ.includes("'"), 'raw quotes gone');
});
