// Skill Parser — parse SKILL.md files into Skill records.
//
// Pure functions — no filesystem or vscode dependency.
// Shared by every agent type (Coding, Browser, Research, …).
//
// This file lives in V2 because the parser is generic:
//   - YAML frontmatter parsing
//   - markdown section extraction
//   - Skill→SkillFrontmatter conversion
//
// Agent-specific loaders (Coding's `skill-loader.ts`) call these
// functions after reading the file from disk.
 ...
 *  Accepts the raw file content, the file path (for error reporting),
 *  and the root directory (for deriving the skill id).
 *  Returns null if the frontmatter is malformed. */
export function parseSkillFile(raw: string, filePath: string, rootDir: string): Skill | null {
...
/** Parse YAML frontmatter (raw text between `---` delimiters) into
 *  a structured SkillFrontmatter object. */
export function parseFrontmatter(raw: string): SkillFrontmatter {
  ...
  for (const line of lines) {
    ...
    if (kvMatch) {
      currentKey = normalizeKey(kvMatch[1]);
    ...
    if (subKvMatch && (currentKey === 'triggers' || currentKey === 'verification')) {
      const subKey = normalizeKey(subKvMatch[1]);
...
/** Parse markdown body into structured SkillSections.
 *  Recognizes: Purpose, Use When, Workflow, Do, Do Not,
 *  Preferred Tools, Verification, References, Examples. */
export function parseSections(content: string): SkillSections {
...
/** Extract basename from a path (no dirname). */
