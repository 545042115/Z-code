...
export interface DesktopSessionSearchOptions {
  query?: string;
...
export class DesktopSessionManager {
  ...
  private save(): void {
    ...
    this.saveTimer = setTimeout(() => {
      this.flushSave();
    }, DesktopSessionManager.SAVE_DEBOUNCE_MS);
  }
  ...
  private flushSave(): void {
    ...
    try {
      ...
      console.error('[DesktopSessionManager] save error:', e);
    }
  }
  ...
  list(opts?: { includeArchived?: boolean }): DesktopChatSession[] {
  ...
  get(id: string): DesktopChatSession | undefined {
  ...
  create(opts?: { title?: string; tags?: string[] }): DesktopChatSession {
    const now = Date.now();
  ...
  rename(id: string, title: string): DesktopChatSession | undefined {
    const session = this.sessions.find((s) => s.id === id);
  ...
  archive(id: string): DesktopChatSession | undefined {
    return this.setArchived(id, true);
  }

  unarchive(id: string): DesktopChatSession | undefined {
    return this.setArchived(id, false);
  }

  private setArchived(id: string, archived: boolean): DesktopChatSession | undefined {
    const session = this.sessions.find((s) => s.id === id);
    if (!session) return undefined;
  ...
  setTags(id: string, tags: string[]): DesktopChatSession | undefined {
    const session = this.sessions.find((s) => s.id === id);
    if (!session) return undefined;
  ...
  appendMessage(sessionId: string, msg: DesktopChatMessage): DesktopChatSession | undefined {
  ...
  search(opts: DesktopSessionSearchOptions = {}): DesktopChatSession[] {
    const {
      query,
